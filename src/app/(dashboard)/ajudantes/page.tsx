...
export default function AjudantesPage() {
  const { data: ajudantes, error, mutate, isLoading } = useSWR<Ajudante[]>('/api/ajudantes', fetcher, {
  ...
  const handleExcluir = async (id: string, nome: string) => {
    if (!confirm(`Tem certeza que deseja excluir o ajudante ${nome}? Esta ação removerá também seus vínculos com coletas e entregas.`)) {
      return;
    }
    setDeletandoId(id);
    try {
      ...
      if (res.ok) {
        ...
        alert(data.error || 'Erro ao excluir ajudante');
      }
    } catch (error) {
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-0.5">
            Gerencie os ajudantes de carga cadastrados no sistema.
          </p>
        </div>
        <Link href="/ajudantes/novo">
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm text-muted-foreground">Carregando ajudantes...</p>
        </div>
        ...
        <Card className="flex flex-col items-center justify-center p-12 border-2 border-dashed bg-card/50">
          ...
          <h3 className="font-semibold text-lg">Nenhum ajudante encontrado</h3>
          <p className="text-sm text-muted-foreground text-center mt-1 max-w-sm">
            {filtro ? 'Ajuste sua pesquisa para encontrar o que procura.' : 'Cadastre seu primeiro ajudante para que ele possa ser atribuído às cargas.'}
          </p>
          {!filtro && (
            <Link href="/ajudantes/novo" className="mt-4">
              <Button size="sm">Cadastrar Ajudante</Button>
            </Link>
          )}
        </Card>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {ajudantesFiltrados.map((a) => (
            <Card key={a.id} className="relative overflow-hidden group border hover:border-primary/35 shadow-sm hover:shadow-md transition-all duration-200 bg-card">
